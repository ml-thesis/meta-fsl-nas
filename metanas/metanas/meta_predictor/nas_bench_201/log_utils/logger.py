...
class Logger(object):
    ...
    def __init__(self, log_dir, seed, create_model_dir=True, use_tf=False):
        ...
        self.tensorboard_dir = self.log_dir / \
            ('tensorboard-{:}'.format(time.strftime('%d-%h',
    ...
    def path(self, mode):
        ...
        if mode == 'model':
            ...
            raise TypeError(
                'Unknow mode = {:}, valid modes = {:}'.format(mode, valids))
    ...
    def scalar_summary(self, tags, values, step):
        ...
        if not self.use_tf:
            ...
            assert isinstance(tags, list) == isinstance(
                values, list), 'Type : {:} vs {:}'.format(type(tags), type(values))
    ...
    def image_summary(self, tag, images, step):
        ...
        for i, img in enumerate(images):
            ...
            img_summaries.append(tf.Summary.Value(
                tag='{}/{}'.format(tag, i), image=img_sum))
