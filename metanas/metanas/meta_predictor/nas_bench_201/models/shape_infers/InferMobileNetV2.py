...
class InvertedResidual(nn.Module):
    def __init__(self, channels, stride, expand_ratio, additive):
        ...
        assert len(channels) in [
            2, 3], 'invalid channels : {:}'.format(channels)
        ...
        if self.additive and channels[0] != channels[-1]:
            self.shortcut = ConvBNReLU(
                channels[0], channels[-1], 1, 1, 1, True, False)
        else:
            self.shortcut = None
    ...
    def __init__(self, num_classes, xchannels, xblocks, dropout):
        ...
        for stage, (t, c, n, s) in enumerate(inverted_residual_setting):
            for i in range(n):
                ...
                module = block(
                    self.xchannels[last_channel_idx], stride, t, additv)
        ...
        features.append(ConvBNReLU(
            self.xchannels[last_channel_idx][0], self.xchannels[last_channel_idx][1], 1, 1, 1))
        assert last_channel_idx + \
            2 == len(self.xchannels), '{:} vs {:}'.format(
