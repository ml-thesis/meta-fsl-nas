...
class Alpha(nn.Module):
    def __init__(self, num_normal, num_reduce, num_op, num_nodes, gsm_soften_eps,
    gsm_temperature, gsm_type, same_alpha_minibatch):
    ...
    def forward(self, batch_size):
        ...
        return {'normal': all_weights_normal_cont,
                'reduce': all_weights_reduce_cont,
                'dis_normal': all_weights_normal_disc,
                'dis_reduce': all_weights_reduce_disc,
                'logit_normal': all_alphas_normal,
                'logit_reduce': all_alphas_reduce}
    ...
    def initialize_architecture_params(self):
        ...
        def create_alphas():
            ...
            for i in range(self.num_nodes):
                input_selector.append(nn.Parameter(
                    torch.zeros(size=[2, i + 2]).cuda(), requires_grad=True))
                op_selector.append(nn.Parameter(
                    torch.zeros(size=[2, num_ops]).cuda(), requires_grad=True))
    ...
    def genotype(self):
        ...
        # TODO: Also adjust these in the network class?
        def _parse(alphas):
    ...
    def log_prob(self, weights):
        def log_q(d, a):
            return torch.sum(torch.sum(
                d * a, dim=-1) - torch.logsumexp(a, dim=-1), dim=0)
